=== Plugin Name ===
Contributors: CoenJacobs
Donate link: http://cnjcbs.com/donate
Tags: comic sans
Requires at least: 2.9
Tested up to: 3.1
Stable tag: 2.1

Any Comic Sans lovers out here? This plugin gives you a complete Comic Sans website, right away! Obviously, this plugin is pun intended.

== Description ==

Changes the font of your WordPress frontend and administration panel to Comic Sans. 

== Installation ==

1. Upload `comic-sans-ftw.php` file to the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Enjoy the Comic Sans power!

== Frequently Asked Questions ==

== Screenshots ==

1. Sample of a Comic Sans powered administration panel

== Changelog ==

= 2.1 =
* Added Comic Sans power for the login too

= 2.0 =
* Added Comic Sans power for the frontend too

= 1.0 =
* Initial release